...
if ( ! function_exists( 'divin_breadcrumb' ) ) :
	function divin_breadcrumb() {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'divin' ); // text for the 'Home' link
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'divin' ); // text for a category page
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'divin' ); // text for a search results page
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'divin' ); // text for a tag page
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'divin' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'divin' ); // text for the 404 page
		...
		if ( ! is_front_page() ) {
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'divin' ) ) ) ), get_the_time( __( 'Y', 'divin' ) ) ) ;
				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'divin' ) ), get_the_time( __( 'm', 'divin' ) ) ) ), get_the_time( __( 'F', 'divin' ) ) );
				echo $before . get_the_time( __( 'd', 'divin' ) ) . $after;

			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'divin' ) ) ) ), get_the_time( __( 'Y', 'divin' ) ) ) ;
				echo $before . get_the_time( __( 'F', 'divin' ) ) . $after;

			} elseif ( is_year() ) {
				echo $before . get_the_time( __( 'Y', 'divin' ) ) . $after;
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'divin' ), max( $paged, $page ) );
